...
@MultipartConfig(fileSizeThreshold = 1024*1024*3, // 메모리 임계크기(이 크기가 넘어가면 레파지토리 위치에 임시파일로 저장됨.)
...
//		System.out.println("─────────────────────────────────────────────────────────────────────────────");
		...
		// Multipart parsing전 파라미터 정보 가져오기
		System.out.println("Multipart parsing 전 sender : " + req.getParameter("sender"));
...
//				fileName = getFileName(part);
				...
				if(fileName != null && !fileName.equals("")) {
					...
					part.write(uploadPath + File.separator + fileName); // 파일업로드...
	...
	/**
	 * Path객체 파싱하여 파일이름 추출하기
	 * @param part 파일이름 파싱할 Part 객체
	 * @return 파일명 : 파일명이 존재하지 않으면 Null값 리턴함
	 */
	private String getFileName(Part part) {
		
		// Content-Disposition : form-data; name="nultiPartServlet"; filename="abc.png"
